...
def analisett(jog1, jog2, datajo):
    ...
    try:
        if stats_j1['over_3_percentage'] > 69:
            if stats_j2['over_3_percentage'] >69:
                if statsh2h['Over3SetsH2h'] > 78:
                    tip = f"{jog1} v {jog2}: Jogo no padrão. Data {datajo}. {urlH2H}."
                    enviartip(tip)
        else:
            print(f"{jog1} v {jog2} Fora do padrão.")
    except KeyError:
        pass

df6 = pd.read_excel('Base de Dados/JogosDoDia.xlsx')

jog1_column = df6['Esquerda']
jog2_column = df6['Direita']
datajo = df6['Data']

for jog1, jog2, datajo in zip(jog1_column, jog2_column, datajo):
    analisett(jog1, jog2, datajo)
